...
from ..partitions import monthy_partition
...
def user_engagement(snowflake: SnowflakeResource) -> None:
    ...
    with open('data/engagement_df.csv', 'w') as output_file:
      engagement_df.to_csv(output_file, index=False)


@asset(
    deps=["dlt_mongodb_embedded_movies"],
    partitions_def=monthy_partition
)
def top_movies_by_month(context, snowflake: SnowflakeResource) -> None:
    ...
    query = f"""
        ...
        from embedded_movies as movies
        join embedded_movies__genres as genres
...
def top_movies_by_engagement():
    ...
    """
    movie_engagement = pd.read_csv('data/engagement_df.csv')
